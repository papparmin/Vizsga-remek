...
export default function AuthModal({ open = true, onClose, onLogin, onRegister }) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Hiba a regisztrációnál");
        ...
        <section className="auth-card">
          <div className="auth-card-top">
            <div className="auth-tabs">
              <button type="button" className={`auth-tab ${mode === "login" ? "active" : ""}`} onClick={() => setMode("login")}>Belépés</button>
              <button type="button" className={`auth-tab ${mode === "register" ? "active" : ""}`} onClick={() => setMode("register")}>Regisztráció</button>
            </div>
            <button type="button" className="auth-x" onClick={onClose || (() => nav("/"))}><CloseIcon /></button>
          </div>
